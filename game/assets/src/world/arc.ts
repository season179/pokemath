...
/** The shipped Woolly map has a gap in the south pen's west wall — the
 * broken fence the arc repairs (regions never resize; the patch swaps one
 * character in place, like the M2B grass swaps). */
export const WOOLLY_PEN_GAP = { x: 18, y: 14 } as const;
...
const WANDERER_SPOTS = [
  { x: 15, y: 19 },
  { x: 18, y: 19 },
  { x: 23, y: 20 },
] as const;

/** The flock, home again: grazing inside the repaired pen. */
const FLOCK_SPOTS = [
  { x: 20, y: 14 },
  { x: 22, y: 15 },
  { x: 21, y: 13 },
...
export function arcCrittersFor(regionId: string, flags: Record<string, number>): ArcCritter[] {
  if (regionId === "meadow/dock") {
    ...
    return [
      { id: DOCK_MOTHLING_ID, x: 1, y: 12, speciesId: "meadow/mothling", topic: "4.1", kind: "battle" },
    ];
  }
