...
test("arc: the repaired pen brings the flock home inside the fence", () => {
  const flock = arcCrittersFor("meadow/woolly", { [FLAG_WOOLLY_PEN]: 2 });
  assert.equal(flock.length, 4);
  assert.ok(flock.every((c) => c.kind === "decor" && c.speciesId === "woolly/fluffball"));
  // Inside the south pen (fence columns x 18 / 25, wall rows 12 / 17).
  const woolly = region("meadow/woolly");
  for (const c of flock) {
    assert.ok(c.x > 18 && c.x < 25 && c.y > 12 && c.y < 17, `${c.id} not in the pen`);
    assert.equal(tileAt(woolly, c.x, c.y), ".");
  }
});
